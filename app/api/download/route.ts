...
const MAX = () =>
  Number(
    process.env.MAX_SOURCE_BYTES || 524288000
  );

const MAX_OUT = () =>
  Number(
    process.env.MAX_OUTPUT_BYTES || 1073741824
  );

const TIMEOUT = () =>
  Number(
    process.env.FFMPEG_TIMEOUT_MS || 300000
  );
...
function runCommand(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    p.on('error', () => {
      ...
      reject(
        new Error(
          `${command} is not installed on the server.`
        )
      );
    });
    ...
    p.on('close', (code) => {
      ...
      if (killed) {
        reject(
          new Error('Download/conversion timed out.')
        );
      } else if (code === 0) {
        resolve();
      } else {
        reject(
          new Error(
            stderr.trim() ||
              `Unable to process media with ${command}.`
          )
        );
      }
    });
  ...
  try {
    ...
    const headers =
      new Headers({
        ...
        'Content-Disposition':
          `attachment; filename="${filenameBase}.${format}"`,
